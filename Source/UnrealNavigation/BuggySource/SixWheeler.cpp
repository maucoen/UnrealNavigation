...
void ASixWheeler::SetBoneArrays()
{
	...
	for (auto& Str : WheelBones)
	{
		...
		SixWheelMesh->SetPhysicsMaxAngularVelocityInDegrees(MaxSpeed, false, Str);
	}
...
void ASixWheeler::MoveForward(float Val)
{
	if (Val != 0)
	{
		...
		for (auto& Bone : WheelBones)
		{
			...
			SixWheelMesh->AddTorqueInRadians(Torque, Bone, true);

			DrawVectors(Bone, Torque);

			FRotator Twist2 = SixWheelMesh->GetBoneQuaternion(Bone, EBoneSpaces::ComponentSpace).Rotator();
			auto PropControl = BogeyTwist * -Twist2.Yaw * 500;

			// proportional control to straighten bogeys
			SixWheelMesh->SetPhysicsAngularVelocityInDegrees(PropControl, false, Bone);
		}
	}
}
	...
	for (auto& Bone : WheelBones)
	{
		if (SixWheelMesh->GetPhysicsLinearVelocity().Size() < 30)
		{
			...
			if (i < 4)
			{
				...
				FRotator Twist2 = SixWheelMesh->GetBoneQuaternion(BogeyBones[i], EBoneSpaces::ComponentSpace).Rotator();
				auto PropControl = BogeyTwist.GetAbs() * -(Twist2.Yaw + powBogey * 45) * 0.1;

				SixWheelMesh->SetPhysicsAngularVelocityInDegrees(PropControl, false, BogeyBones[i]);
			}
		}
		...
		else if (i < 4)
		{
			...
			SixWheelMesh->SetPhysicsAngularVelocityInDegrees(powBogey2*BogeyTwist, false, BogeyBones[i]);
		}
		++i;
	}
}
...
void ASixWheeler::DrawVectors(FName Bone, FVector Torque)
{
	if (bDraw)
	{
		...
		Torque = Torque.RotateAngleAxis(90, GetActorUpVector()) / 500000;
