...
namespace px4_msgs
{
	...
	public:
        ...
        virtual void FromJson(TSharedPtr<FJsonObject> JsonObject) override 
		{
            ...
            y_coeffs.Empty();
            PtrArr.Empty();
			PtrArr = JsonObject->GetArrayField(TEXT("y_coeffs"));
			for (auto &ptr : PtrArr)
            {
            ...
            z_coeffs.Empty();
            PtrArr.Empty();
			PtrArr = JsonObject->GetArrayField(TEXT("z_coeffs"));
			for (auto &ptr : PtrArr)
            {
            ...
            yaw_coeffs.Empty();
            PtrArr.Empty();
			PtrArr = JsonObject->GetArrayField(TEXT("yaw_coeffs"));
			for (auto &ptr : PtrArr)
            {
            ...
            t_trans.Empty();
            PtrArr.Empty();
			PtrArr = JsonObject->GetArrayField(TEXT("t_trans"));
			for (auto &ptr : PtrArr)
            {
		...
		{
			return TEXT("nope");// + FString::SanitizeFloat(X) +
